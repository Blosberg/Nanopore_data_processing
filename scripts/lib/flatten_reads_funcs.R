...
{ 
  ...
  read_GRL_in_splitby_startpos <- split(   read_GR_in_Ns_removed, 
    ...
    read_GRL_in_splitby_startpos_flattened  <- lapply( read_GRL_in_splitby_startpos, 
                                                       function(x) flatten_GR_at_spec_position( readposn_GR_in = x,
                                                                                                chr_in         = chr,
                                                                                                strand_in      = str
                                                                                                )
    ...
    # Now turn this list back into a regular GRanges object, so that it can be used as a single 
    # element of a higher-order list ( possibly grouped by, e.g., read_index )
    # return ( read_GRL_in_splitby_startpos_flattened  )
...
{ 
  if( length( readposn_GR_in ) == 1)
    ...
    temp            =  readposn_GR_in[1];
      
    S               = start(  temp )
    E               = end (   temp )

    reference_kmer  = temp$reference_kmer
    read_index      = temp$read_index
    event_index     = temp$event_index  # N.B. only the first "event" is kept here.
    
    model_kmer      = temp$model_kmer
    model_mean      = temp$model_mean
    model_stdv      = temp$model_stdv 
  ...
  result <- GRanges( seqnames        = chr_in,
                    ...
                    read_index = read_index,
                     # ^ all of the above should be uniform for this read at this position.                    
  
                     event_index =     event_index,
                     # this represents an arbitrary selection of the "first" 
